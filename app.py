...
if choice == "Main":
    ...
    st.image("D:/ml/Predictive-Maintenance-System-Monitoring-main/assets/Untitled.png", caption="Predictive Maintenance System", use_column_width=True)
...
if choice == "Prediction":
    ...
    # Call your prediction function to get the results
        result1, result2 = prediction(type, rpm, torque, tool_wear, air_temp, process_temp)
    
        failure_reasons = []
    
    # Defining conditions for potential failure reasons
        if rpm > 1600:
            failure_reasons.append("High rotational speed (RPM) is exceeding the safe limit.")
    
        if torque > 45:
            failure_reasons.append("Excessive torque is being applied.")
    
        if tool_wear > 18000:
            failure_reasons.append("Tool wear has exceeded the recommended threshold, indicating the tool is near the end of its life.")
    
        if air_temp > 30:
            failure_reasons.append("Air temperature is too high, which can lead to overheating.")
    
        if process_temp > 350:
            failure_reasons.append("Process temperature is excessively high, which could damage the machine.")

    # Outputting the results
        if result1 != "No Failure":  # If there is a failure
            st.write("Machine Failure?: ", result1)
            st.write("Type of Failure: ", result2)
        
        # Display the reasons for failure
            if failure_reasons:
                st.write("Reason for Failure:")
                for reason in failure_reasons:
                    st.write("- ", reason)
            else:
                st.write("No specific reason identified, but the machine has failed.")
        
            st.write("Schedule a maintenance for the machine.")
    
        else:  # If there is no failure
            st.write("Machine Failure?: No failure detected")
            st.write("The machine is running normally.")
